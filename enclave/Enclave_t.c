...
typedef struct ms_generate_keys_t {
	sgx_status_t ms_retval;
} ms_generate_keys_t;
...
static sgx_status_t SGX_CDECL sgx_generate_keys(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_generate_keys_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_generate_keys_t* ms = SGX_CAST(ms_generate_keys_t*, pms);
	sgx_status_t status = SGX_SUCCESS;



	ms->ms_retval = generate_keys();
...
SGX_EXTERNC const struct {
	...
	4,
	{
		{(void*)(uintptr_t)sgx_say_something, 0},
		{(void*)(uintptr_t)sgx_generate_keys, 0},
